...
export default async function SeekerMatchDetailPage({ params }: Props) {
  ...
  if (!match || match.seekerId !== profile.id) redirect('/applications')
  ...
  const hasSeekerReport = match.workReports.some((r) => r.reporterType === 'SEEKER')

  return (
    <>
      <Header role="SEEKER" email={user.email} />
      <PageContainer maxWidth="sm">
        <SectionHeading subtitle={match.nursery.nurseryName}>
          マッチング詳細
        </SectionHeading>
        ...
        <Box sx={{ p: 2, bgcolor: '#FAFAFA', borderRadius: 2, border: '1px solid #E0E0E0', mb: 3 }}>
          ...
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
            📍 {match.nursery.nurseryName}（{match.nursery.area}）
          </Typography>
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
        ...
        <MatchActions
          matchId={match.id}
          status={match.status}
          hasSeekerReport={hasSeekerReport}
          chatOpen={chatOpen}
