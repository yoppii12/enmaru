...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (loginRes.ok && loginData.redirectPath) {
    ...
    <PageContainer maxWidth="sm" py={4}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5, mb: 3 }}>
        <RoleCard
          ...
          icon={<ChildCareIcon sx={{ fontSize: 36, color: role === 'SEEKER' ? '#F4A7B9' : '#AAAAAA' }} />}
          title="保育士"
        ...
        <RoleCard
          ...
          icon={<ApartmentIcon sx={{ fontSize: 36, color: role === 'NURSERY' ? '#F4A7B9' : '#AAAAAA' }} />}
          title="保育園"
      ...
      <Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center' }}>
        すでにアカウントをお持ちの方は{' '}
        <MuiLink component={NextLink} href="/login" color="primary" underline="hover">
          ログイン
        </MuiLink>
      </Typography>
    </PageContainer>
