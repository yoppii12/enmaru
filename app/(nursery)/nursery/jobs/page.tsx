      ...
      <PageContainer>
        ...
        {jobs.length === 0 ? (
          <Box sx={{ textAlign: 'center', py: 8 }}>
            <Typography color="text.secondary" sx={{ mb: 2 }}>まだ募集情報がありません</Typography>
            <Button component={Link} href="/nursery/jobs/new" variant="contained">
              最初の募集を作成する
            </Button>
          </Box>
        ) : (
          <>
            {openJobs.length > 0 && (
              <Box sx={{ mb: 3 }}>
                <Typography variant="subtitle2" sx={{ mb: 1.5, color: '#666666' }}>公開中（{openJobs.length}件）</Typography>
            ...
            {closedJobs.length > 0 && (
              <>
                <Divider sx={{ my: 2 }} />
                <Box>
                  <Typography variant="subtitle2" sx={{ mb: 1.5, color: '#AAAAAA' }}>終了済み（{closedJobs.length}件）</Typography>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
...
function JobRow({ job }: { job: { id: string; title: string; workDate: Date; workTimeStart: string; workTimeEnd: string; hourlyWage: number | null; status: string } }) {
  return (
    <Box
      sx={{
        p: { xs: 1.5, md: 2 },
    ...
    >
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 0.5 }}>{job.title}</Typography>
        <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
          ...
          {job.hourlyWage && (
            <Typography variant="caption" color="text.secondary">時給{job.hourlyWage.toLocaleString()}円</Typography>
          )}
        </Box>
      </Box>
